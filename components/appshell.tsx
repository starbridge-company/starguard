...
// Garante que o destino do link é um caminho interno relativo, evitando que
// esquemas perigosos (ex.: "javascript:") ou URLs absolutas/externas sejam
// injetados em `href` (mitiga XSS - CWE-79).
const isSafeInternalHref = (href: string) => href.startsWith("/") && !href.startsWith("//");
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const renderLink = (item: NavItem) => (
    <Link
      key={item.href}
      className={item.match(pathname)}
      href={isSafeInternalHref(item.href) ? item.href : "#"}
    >
