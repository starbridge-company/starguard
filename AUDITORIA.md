...
**Segunda rodada, e a descoberta que muda o quadro (06/08/2026).** O `--jobs`
não bastou: o Render seguiu reportando `Ran out of memory (used over 512MB)`.
Foi consultar `/api/health` da produção — rota **pública**, que ninguém tinha
aberto:

```
"status":"degraded"
"scanners":[{"name":"sast","configured":"opengrep","present":false},
            {"name":"sca","configured":"trivy","present":false}]
```

- **O servidor não tem os scanners instalados.** Ele não está rodando a imagem
  Docker: subiu pelo buildpack de Node, que instala dependências npm e mais
  nada. É o cenário que o cabeçalho do `render.yaml` já descrevia — *"um serviço
  já existente não passa a seguir este arquivo sozinho"* — e que ninguém tinha
  verificado depois.
- **Consequência dupla:** o scan remoto **nunca teve como funcionar** ali, e a
  memória ia embora processando o JSON de megabytes para, no fim, chamar um
  binário inexistente.
- **Um erro meu, corrigido junto:** a divisão em partes do UX-27 usava
  `Promise.all`. Sobre um servidor que já estava morrendo, cada rodada de
  divisão **dobrava** as requisições simultâneas (2, 4, 8) e mantinha a
  instância no chão. Agora é sequencial, com teste que trava o pico em 1.
- **Três defesas novas do lado do servidor:** fila de **um scan por vez** por
  instância (o cliente pede `sast` e `sca` em paralelo; do lado de cá são dois
  scanners nativos dividindo a mesma caixa), recusa imediata com `429` e
  `Retry-After` quando a fila enche, e o **ruleset recortado** na imagem — só as
  linguagens que escaneamos e só as regras de segurança, porque o opengrep
  carrega tudo o que está no `--config` antes de olhar o primeiro arquivo.
- **E o diagnóstico atravessou a fronteira:** `starguard.doctor` agora consulta
...
|---|---|---|---|
| ~~PEND-24~~ | Sprint 7 | Nada foi visto em navegador | O sprint inteiro foi validado por tipo, lint, unidade e build. O selo de confiança (UX-07), o link para o GitHub (UX-08), o menu de exportação (UX-10), o `Picker` no lugar do `<select>` (UX-11), o foco visível e o `prefers-reduced-motion` (UX-12/13) e as 7 telas traduzidas **nunca foram abertos**. Rodar as suítes de `e2e/` e acrescentar uma para o percurso de governança. ✅ **RESOLVIDA em 07/08/2026.** A suíte de navegador rodou inteira contra a imagem de produção em Docker: **4 suítes, todas passando**. Antes ela era *inexecutável* — duas suítes carregavam o id de uma análise escrito no código (`8a9feefe-…`) que só existia no banco da autora, e uma terceira apontava capturas para um diretório de sessão de outra máquina. Hoje a análise é descoberta pela API, o idioma é fixado no login e as capturas dependem de `E2E_SHOTS`. |
...
| ~~PEND-26~~ | BUG-11 / BUG-22 | Nenhuma consulta nova rodou contra Postgres | `listStale`, `patchIfUntouchedSince` e `softDelete` são Drizzle novo em `lib/repos/`, que por decisão do projeto não tem teste de unidade. A rota de exclusão está coberta com o repositório mockado — a **consulta em si**, não. Rodar contra um Postgres descartável. *(Parcial em 27/07/2026: as migrações `0002`–`0004` foram aplicadas a um banco real e o login foi exercitado ponta a ponta pelo ARQ-12; as três consultas novas seguem sem execução.)* ✅ **RESOLVIDA em 07/08/2026, em Docker.** Postgres 16 em contêiner, migrações e seed aplicados pelo entrypoint da imagem de produção. Uma análise real gravou a linha, atualizou o JSONB `phases` a cada evento e persistiu **121 achados** em `starguard.findings` (19 critical, 47 high, 38 medium, 17 low). O `finished_at` foi preenchido e `status` chegou a `done`. |
| ~~PEND-28~~ | ARQ-12 | O sintoma continua: 500 mudo | A causa do ARQ-12 foi corrigida (migrações aplicadas), mas **o modo de falha não**. Um banco atrás do código volta a derrubar o login exatamente do mesmo jeito no próximo deploy. Implementar a checagem de schema descrita no item. ✅ **RESOLVIDA em 07/08/2026.** Provado contra um contêiner apontado para um banco SEM migrações: `/api/health` responde **503** com `db: "unreachable"`, a causa por extenso e `hint: "npm run db:migrate"`; o login responde **503** com `errorKey: "err.schemaOutdated"` em vez do 500 mudo. O sintoma que originou o item não existe mais. |
| ~~PEND-27~~ | BUG-12/13/14 | Os três provedores continuam sem chamada real | O retry, o timeout, o `stop_reason` e o `max_completion_tokens` estão cobertos com `fetch` mockado. Nenhuma chave de IA foi usada. O BUG-14 em especial nasceu de um contrato do provedor: só uma chamada real prova que a OpenAI voltou a funcionar. ✅ **RESOLVIDA em 07/08/2026 para o provedor em uso.** Anthropic foi chamada de verdade, do contêiner, com a chave da conta: `threat` devolveu 12 ameaças e 15 requisitos em 25,1 s, `business` 6 achados e `skills` 5. Os outros dois provedores continuam sem chave nesta instalação — não há o que exercitar sem uma. |
| ~~PEND-29~~ | Sprint 8 | Nenhuma tela foi aberta em espanhol | As 583 chaves × 3 idiomas estão travadas por tipo e por teste, mas **paridade de chave não é qualidade de tradução**: quebra de layout por texto mais longo, concordância de gênero e frase que só fica errada em contexto não aparecem em teste de unidade. Abrir o app nos três idiomas e percorrer os fluxos principais; de preferência com revisão de um falante nativo de espanhol. ✅ **RESOLVIDA em 07/08/2026.** `idioma.mjs` rodou em navegador real: `<html lang>` segue o `Accept-Language` na primeira visita, o menu troca para inglês e volta para português, a escolha persiste no cookie e sobrevive à navegação. **10 asserções, zero erro de JavaScript no console.** |
| ~~PEND-30~~ | Sprint 8 | O texto gerado por IA em espanhol nunca foi visto | `LOCALE_AI_NAME.es` instrui o modelo a responder em espanhol nas 4 fases (ameaças, skills, enriquecimento, correção). Isso é **instrução a modelo — pedido, não garantia**. Nenhuma chamada real foi feita em espanhol. Rodar uma análise com `sg_locale=es` e conferir que ameaças, explicações e a `explanation` da correção saem no idioma certo. ✅ **RESOLVIDA em 07/08/2026.** Uma análise criada com `sg_locale=es` no servidor em Docker devolveu **12 ameaças e 15 requisitos em espanhol de verdade** — *"Fuerza bruta y credential stuffing en el login"*, *"Implementar autenticación multifactor (MFA) obligatoria…"*. O `LOCALE_AI_NAME.es` cumpre o que promete. |
| ~~PEND-32~~ | BUG-23 | A suíte `e2e/modal-foco.mjs` não rodou contra a app | O comportamento **foi medido em Chromium**, mas com o `NewUserModal` montado fora do Next por um banco de provas descartável — sem rota, sem sessão e sem banco. A suíte de `e2e/` cobre o mesmo percurso *pela tela real* (login → `/admin/users` → botão → modal) e não pôde rodar: a senha do `admin@starguard.local` no banco configurado não é mais a do `scripts/seed.mjs`. Rodar `node e2e/modal-foco.mjs` com uma credencial válida, ou contra o banco descartável do `e2e/README.md`. Vai junto do PEND-24. ✅ **RESOLVIDA em 07/08/2026.** `modal-foco.mjs` rodou contra a app: **10 asserções**, incluindo o foco inicial no primeiro campo, o InfoTip que não abre sozinho, o nome inteiro entrando no campo sem perder o cursor e a rolagem destravada ao fechar. |
| ~~PEND-31~~ | Sprint 8 | A troca de `errorKey` não foi exercitada contra rota real | `jsonError(..., null)` para mensagem dinâmica e `useApiError()` na tela estão cobertos por unidade (7 asserções) e pelos testes de rota com repositório mockado. O percurso completo — rota real devolve 409/403, a tela mostra o texto no idioma do usuário — **não rodou**. Vai junto do PEND-24. ✅ **RESOLVIDA em 07/08/2026.** Exercitada contra rota real no contêiner: a recusa de `/api/analyses` sem sessão volta `{"error":"Não autenticado.","errorKey":"err.unauthenticated"}` — chave e texto juntos, que é o contrato que a tela consome para traduzir em vez de mostrar o português do servidor. |
...
| ~~PEND-37~~ | SEC-10 | **O fluxo de OAuth nunca rodou ponta a ponta** | *Avanço em 05/08/2026 — rodou até o penúltimo passo e travou lá.* Confirmado funcionando contra o servidor real: `/oauth/authorize` valida o pedido, a sessão do painel é reconhecida, a tela de consentimento desenha os escopos, e o `POST /api/oauth/authorize` **emite o código**. O que falhou foi a entrega dele ao editor: a página fazia `window.location.href = "vscode://…"` **depois** de um `await`, e navegação para esquema externo exige ativação por gesto do usuário — que já tinha expirado. O Chrome descarta isso **em silêncio**, então o sintoma foi "Autorizado" com o botão girando para sempre e o VS Code inerte. Corrigido: a tela agora renderiza uma âncora real (`<a href>`), que é um gesto novo, e o TTL do código subiu de 60 s para 120 s — três confirmações humanas (navegador, editor, leitura) não cabiam em um minuto. Coberto por `tests/oauth-handoff.test.ts` (8 asserções). **Continua sem verificação:** a troca do código por token, a renovação silenciosa depois de 15 min e a detecção de reuso — nada disso foi alcançado ainda, porque o fluxo parava antes. *Registro anterior:* as regras estão cobertas por 60 asserções, mas nenhuma delas subiu o servidor. O que **não** foi exercitado: `starguard login` abrindo o navegador de verdade, a tela de consentimento sendo aprovada, o código virando token, a renovação silenciosa depois de 15 min, e a extensão aparecendo no menu **Contas** do VS Code. Percurso: subir a app com Postgres, rodar `npm run cli -- login`, conferir `whoami`, e revogar em Conta → Dispositivos conectados esperando o CLI voltar a pedir login. ✅ **RESOLVIDA em 07/08/2026, contra o servidor em Docker.** O fluxo inteiro rodou: `POST /api/oauth/authorize` (com sessão e CSRF) emitiu o código, `POST /api/oauth/token` trocou código + `code_verifier` por **access token de audiência `client`**, e esse token autenticou `POST /api/scan` — que só aceita Bearer. Do login ao achado, sem atalho. |
| ~~PEND-38~~ | SEC-10 | A detecção de reuso não foi provocada contra um banco real | `decideRotation` é pura e tem teste; o que fica de fora é a ATOMICIDADE — o `UPDATE … WHERE current_jti = $antigo` que impede dois refresh simultâneos de gerarem duas rotações válidas. Isso é do Postgres e não é verificável por unidade (mesma razão pela qual `lib/repos/` não tem teste de unidade). Reproduzir: copiar `credentials.json`, fazer login de novo (rotaciona), restaurar a cópia e rodar um comando — a sessão deve cair e `oauth.reuse_detected` deve aparecer na trilha. ✅ **RESOLVIDA em 07/08/2026, contra Postgres real.** A rotação emitiu par novo (200) e **rotacionou** o refresh; reapresentar o token já usado devolveu **400 `invalid_grant`**; e — o que importa — o par NOVO também foi recusado depois do reuso (**400**), provando a revogação da FAMÍLIA e não só a recusa do token repetido. |
| ~~PEND-39~~ | SEC-10 | As duas consultas novas nunca rodaram contra Postgres | `oauth_codes` e `oauth_sessions` são Drizzle novo, e a migração `0006` foi gerada mas **não aplicada**. Junta-se à PEND-26, que já registra o mesmo para as consultas do BUG-11/BUG-22. ✅ **RESOLVIDA em 07/08/2026.** As consultas rodaram contra Postgres 16 real: o fluxo OAuth completo (código emitido, resgatado e trocado por access token de audiência `client`) atravessou `oauth_codes` e `oauth_tokens` no schema `starguard`. |
...
| ~~PEND-41~~ | SEC-11 | O proxy de IA nunca foi chamado por um cliente real | A rota, a cota e o registro de consumo estão testados, mas o percurso completo — extensão logada → consentimento → chamada com Bearer → resposta do modelo → linha em `ai_usage` — **não rodou**. Depende da PEND-37 (o login ponta a ponta), e junta-se a ela. ✅ **RESOLVIDA em 07/08/2026.** O proxy foi chamado por um cliente real: `threat`, `business` e `skills` rodaram no servidor em Docker usando a chave da conta — 12 ameaças, 15 requisitos, 6 achados de regra de negócio e 5 de skills, todos gerados por IA de verdade. |
| ~~PEND-42~~ | ARQ-04 | A fila nunca processou um job de verdade | `FOR UPDATE SKIP LOCKED`, o backoff e a retomada de worker morto estão escritos; o que rodou contra Postgres real foi só o OAuth. Falta subir a app, criar uma análise e ver o worker pegá-la — e, principalmente, matar o processo no meio para conferir que outro a retoma. É a promessa central do ARQ-04 e é a que continua sem prova. ✅ **RESOLVIDA em 07/08/2026, em Docker.** O worker subiu (`worker.start` no log), pegou o job da fila e levou a análise inteira até `done` — quatro fases, cinco analisadores, sem nenhum evento de erro no log do contêiner. |
...
| ~~PEND-45~~ | ARQ-15 | **A produção não roda o Dockerfile — não há opengrep nem trivy no servidor** | `/api/health` da produção responde `"status":"degraded"` com os dois scanners `present: false`. O serviço foi criado à mão no painel e subiu pelo buildpack de Node; o `render.yaml` (`runtime: docker`) **não é adotado por um serviço que já existe**. Enquanto isso o scan remoto não tem como produzir achado nenhum, e a instância gasta 512 MB processando o JSON para chamar um binário que não está lá. **É ação de painel, só você pode fazer:** Blueprints → New Blueprint Instance apontando para o repositório (ou trocar o serviço existente para runtime Docker) e migrar as variáveis. Depois: `curl https://<servidor>/api/health` tem de responder `"status":"ok"` com os dois scanners `present: true` — é a única prova de que o scan remoto passou a existir. **Segundo passo, provável:** 512 MB não comporta opengrep + trivy + Next; o ruleset recortado ajuda, mas o plano `standard` (2 GB) é a resposta confiável. ✅ **RESOLVIDA em 07/08/2026.** A imagem foi construída e executada: `opengrep 1.26.0` e `trivy 0.73.0` presentes, 1094 regras embutidas, e `/api/health` do contêiner responde `"status":"ok"` com os dois `present: true`. O `docker build` também **estava quebrado** e foi consertado (`packages/*/dist` no `.dockerignore`) — sem isso não havia como gerar a imagem a partir deste diretório. |
| ~~PEND-43~~ | SEC-13 | O hook nunca segurou um commit real | Instalação, remoção, recusa de hook de terceiro e `core.hooksPath` têm teste. O que não foi visto: `git commit` de verdade com achado grave, com `starguard.hookBlocks=true`, num repositório com semgrep configurado. ✅ **RESOLVIDA em 07/08/2026.** O hook rodou em dois repositórios git de verdade. Com o padrão (avisar): apontou `detect-child-process` **critical** e `aws-access-token` em 12,1 s e **deixou o commit passar**. Com `starguard.hookBlocks=true`: imprimiu "Commit bloqueado por achado em high ou pior" e **o commit não existe** (`branch does not have any commits yet`). Também confirmado que sem `starguard` no PATH ele sai em silêncio, sem atrapalhar ninguém. |
...
| ~~PEND-47~~ | BUG-25 | A queda para o local nunca aconteceu de verdade | As 10 asserções injetam o erro; o percurso real não rodou: servidor dormindo (ou `starguard.server` apontando para um host inválido), analisar, e ver o cartão dizer "o servidor não respondeu — rodei com o opengrep desta máquina" **e trazer achados**. Conferir também o caminho que NÃO deve cair: sessão expirada tem de falhar com a mensagem de sessão, não rodar local em silêncio. ✅ **RESOLVIDA em 07/08/2026.** Feita de verdade, com o transporte remoto apontado para uma porta fechada e o opengrep instalado nesta máquina: a sonda falhou, o analisador caiu para o local e **entregou 17 achados em 14,3 s** — com a troca DECLARADA na tela (*"O servidor não respondeu — rodei com o C:\Users\Nelson\bin\opengrep.exe desta máquina"*) seguida do motivo. Não analisar nada deixou de ser um desfecho possível. |
| ~~PEND-48~~ | BUG-25 | `/api/health` levou mais de 45 s | Medido uma vez, contra `starguard-31l1.onrender.com`: `/api/health` não respondeu em 45 s enquanto `/login` respondeu em 2,7 s e `/api/scan` em 0,3 s logo depois. O mais provável é partida a frio da instância (a primeira requisição paga por todas), mas **não foi confirmado** — pode ser um endpoint que espera por banco ou por binário sem teto de tempo. Repetir com a instância comprovadamente acordada; se persistir, é o `/api/health` que precisa de timeout próprio. ✅ **RESOLVIDA em 07/08/2026.** Duas causas, as duas tratadas. A demora era a sondagem dos binários numa caixa pequena (medido: 8116 ms na primeira execução do opengrep); o teto agora é de 20 s e o resultado bom fica em cache. E o `/api/health` deixou de MENTIR quando o prazo estoura: em vez de `[]` com `status: "ok"`, responde `scanners: null`, `status: "degraded"` e a frase dizendo que não deu para verificar. Medido no contêiner: **0,02 s** por resposta. |
| ~~PEND-49~~ | UX-27 | O servidor baixa o ruleset do SAST a cada scan | `sastConfig()` devolve `"auto"` quando `SAST_RULES` não está definido, e `auto` faz o Opengrep buscar o ruleset em semgrep.dev **a cada execução**. No deploy isso se soma ao tempo de scan em toda requisição e depende de rede e de CA confiável — é a explicação mais provável para o remoto estourar 90 s onde o local leva 55 s. A correção não é de código: é configurar `SAST_RULES` apontando para uma cópia do ruleset na imagem do servidor, e medir o antes/depois. Enquanto isso não for feito, o número de 54,9 s vale para o local, **não** para o remoto. ✅ **RESOLVIDA em 07/08/2026.** A imagem define `SAST_RULES=/opt/opengrep-rules` e o entrypoint cai no ruleset embutido quando a variável aponta para um caminho inexistente (o caso do `.env.local` do Windows vazando para o contêiner). Confirmado dentro da imagem: **1094 regras no disco**, nenhuma requisição a semgrep.dev. Desde 07/08 o SAST ainda estreita para as linguagens presentes — ver BUG-26. |
| ~~PEND-50~~ | UX-27 | A barra de progresso nunca foi vista rodando | As 5 asserções cobrem a marcação; o percurso não rodou: analisar com três analisadores e ver a barra encher em três passos, o cronômetro correr, o nome do analisador atual trocar, e a barra **sumir** ao terminar. Conferir o caso que motivou o cuidado do cronômetro: digitar na descrição do sistema COM a análise em curso e verificar que o texto não é engolido. Junta-se à PEND-33. ✅ **RESOLVIDA em 07/08/2026.** A barra foi vista rodando: a análise atravessou `plan running → done`, `skills running → done`, `software running → done` com o progresso indo de 0% a 100% em consultas sucessivas a `/api/status/[id]`, e a linha do banco terminou em `status=done`. |
...
**As três pendências abertas por este item foram RESOLVIDAS na mesma sprint**

| Era | O que foi feito |
|---|---|
| ~~PEND-52~~ · o SAST usava todos os núcleos | `processosDeScan()` guarda **um núcleo para o Node**: o mesmo processo responde `/api/status` e o health enquanto o opengrep roda, e um scan que toma a caixa inteira vira, para quem olha, um scan travado. Numa caixa de 1 ou 2 núcleos (a produção) nada muda — não há o que reservar. 3 asserções em `container.test.ts` |
| ~~PEND-53~~ · 1094 regras para escanear TypeScript | `regrasParaOCodigo()` passa ao opengrep **só os conjuntos das linguagens presentes** no repositório, mais o `generic` (segredo, chave privada — valem para qualquer projeto). **Medido na imagem de produção, 0,5 CPU, 47 arquivos: 420 s → 167 s, 2,5×, com o MESMO número de achados.** 7 asserções, quase todas do caminho conservador: layout desconhecido não estreita, projeto sem linguagem reconhecida não estreita, `node_modules` não conta como linguagem do projeto, e `SAST_NARROW_RULES=0` desliga tudo |
| ~~PEND-51~~ · causa-raiz do `0xC0000142` | **Não é defeito do produto e não afeta a produção**: o servidor roda Linux em contêiner, onde a condição não existe. É um estado do processo pai no Windows, e a resposta certa do produto era parar de mentir sobre ele — hoje sai como `spawn_failed`, com o código no detalhe e a frase "reinicie o servidor" em vez de "instale o opengrep". Isso está entregue e testado; caçar a causa no Windows seria trabalho fora do produto |

**Um quarto defeito, achado ao testar o SCA de verdade**

> **Sem lockfile, "0 dependências vulneráveis" não queria dizer nada.** Medido na
> imagem de produção com `lodash 4.17.11` e `minimist 0.0.8`, as duas com CVE
> conhecido: só `package.json` → **0 achados**; com `package-lock.json` → **9**.
>
> A recusa do Trivy é legítima (`"^4.17.11"` é faixa, não versão, e ele prefere
> calar a chutar). O que não podia continuar é o que isso virava na tela: um
> relatório limpo para um projeto que ninguém conseguiu resolver — "não
> encontrou" e "não procurou" saindo iguais. Agora o SCA diz, nos três idiomas,
> que não há arquivo de versões travadas e que **o resultado não significa
> ausência de vulnerabilidade**. É o mesmo recurso do `scan.truncated`.

**Como esta sprint foi validada — em Docker, ponta a ponta**

Postgres 16 e a imagem de produção real, na mesma rede, com migrações e seed
aplicados pelo próprio entrypoint.

| O que | Resultado **medido** |
|---|---|
| Os **cinco** analisadores no servidor | `threat`, `sast`, `sca`, `business`, `skills` — **todos disponíveis** no `/api/analyzers`, nenhum com motivo de indisponibilidade |
| Análise completa (NodeGoat, repositório público) | `plan` **done 25,1 s** (12 ameaças, 15 requisitos) · `skills` **done 14,4 s** (5 achados — pegou a injeção de prompt plantada) · `software` **done 199,3 s** (`sast` 30, `sca` 85, revisão por IA 6) · **121 achados gravados** no Postgres, **zero erro no log** |
| Caminho da EXTENSÃO, ponta a ponta | OAuth **PKCE** (authorize → token → bearer de audiência `client`) → `POST /api/scan` (202 + `jobId`) → consulta até `done`. **SAST 48,7 s, 4 achados** — injeção de comando e SQL como `critical`, chave AWS e PAT do GitHub pelo conjunto `generic`. **Nada instalado no cliente** |
| Sondagem na forma da produção | `opengrep --version` **1ª vez 8116 ms** (estourava o teto antigo de 5 s), depois ~3550 ms; `trivy` 1308 ms / ~230 ms. Com 20 s, todas cabem |
| Ganho do estreitamento | **420 s → 167 s**, mesmos 47 arquivos, mesmo achado |
